...
const HERO_IMAGE = "https://www.fakhruddinproperties.com/wp-content/uploads/2026/01/Treppan-Living-PRIVE-Pool01.webp";
      ...
      <div className="max-w-[1300px] mx-auto px-6 md:px-12 lg:px-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="intro-left">
            ...
            <p className="font-seitu text-base leading-relaxed mb-5" style={{ color: "#4a5568" }}>
              Fakhruddin International represents our commitment to delivering world-class real estate beyond the UAE. With completed projects across the United Kingdom and active developments in Uganda, we bring the same standard of excellence that defines our home market to every new geography.
            </p>
            <p className="font-seitu text-base leading-relaxed" style={{ color: "#4a5568" }}>
              Each international project is approached with deep local understanding, long-term vision, and an unwavering focus on quality — creating spaces that resonate with communities for generations.
            </p>

            <div className="mt-10 grid grid-cols-2 gap-4">
              {["United Kingdom", "Uganda", "Strategic Partnerships", "Long-Term Value"].map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: `${PRIMARY}18`, border: `1px solid ${PRIMARY}40` }}>
                    <CheckmarkIcon />
                  </div>
                  <span className="text-sm font-seitu" style={{ color: CHINESE_BLACK }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="intro-right grid grid-cols-1 sm:grid-cols-2 gap-5">
            {pillars.map((pillar, idx) => (
              ...
              >
                ...
                <p className="text-sm leading-relaxed font-seitu" style={{ color: "#718096" }}>{pillar.desc}</p>
                <motion.div
...
function RegionSection({
  ...
  useGSAP(() => {
    gsap.fromTo(".region-header-" + region.replace(/\s/g, ""), { y: 40, opacity: 0 }, {
  ...
  return (
    <section ref={ref} className="relative w-full py-24 overflow-hidden" style={{ background: flipped ? `${CHINESE_BLACK}` : WHITE }}>
      {/* Cover image with parallax feel */}
      <div className="relative w-full mb-16 overflow-hidden" style={{ height: "380px" }}>
        <motion.img
          src={cover}
          alt={region}
        ...
        />
        <div className="absolute inset-0" style={{ background: `linear-gradient(to top, ${flipped ? CHINESE_BLACK : WHITE} 0%, transparent 60%)` }} />
        <div className="absolute inset-0" style={{ background: `linear-gradient(to right, ${flipped ? CHINESE_BLACK : WHITE}80 0%, transparent 50%)` }} />

        {/* Region label */}
        <div className={`absolute ${flipped ? "right-12 text-right" : "left-12 text-left"} bottom-10`}>
          <span className={`region-header-${region.replace(/\s/g, "")} block text-[10px] tracking-[0.45em] uppercase mb-3 font-seitu opacity-0`} style={{ color: accent }}>
            International Portfolio
          </span>
          <h2 className="font-faculty text-5xl md:text-7xl" style={{ color: WHITE, fontWeight: 400 }}>
            {region}
          </h2>
          <div className="w-12 h-[2px] mt-4" style={{ background: accent, marginLeft: flipped ? "auto" : 0, marginRight: flipped ? 0 : "auto" }} />
        </div>
      </div>

      {/* Description */}
      <div className="max-w-[1300px] mx-auto px-6 md:px-12 lg:px-20 mb-14">
        <div className={`max-w-2xl ${flipped ? "ml-auto text-right" : ""}`}>
          <p className="text-base leading-relaxed font-seitu" style={{ color: flipped ? PALE_SILVER : "#4a5568" }}>
            {description}
          </p>
        </div>
      </div>
...
function MarqueeDivider() {
  const items = ["United Kingdom", "Uganda", "East Africa", "London", "Kampala", "Global Excellence", "Landmark Design", "Purposeful Growth"];
      ...
      >
        {[...items, ...items].map((item, i) => (
          <div key={i} className="flex items-center gap-10 flex-shrink-0">
            <span className="text-[11px] tracking-[0.35em] uppercase font-seitu" style={{ color: `${PRIMARY}` }}>{item}</span>
...
function NumbersStrip() {
  ...
  useGSAP(() => {
    gsap.fromTo(".numbers-item", { y: 40, opacity: 0 }, {
  ...
  const stats = [
    ...
    { value: "12+", label: "International Projects" },
    { value: "2", label: "Countries" },
    { value: "1,500+", label: "Homes Delivered" },
  ];

  return (
    <section ref={ref} className="relative w-full py-24 overflow-hidden" style={{ background: `linear-gradient(135deg, ${MSU_GREEN} 0%, ${DEEP_AQUAMARINE} 100%)` }}>
      {/* Decorative grid */}
      <div className="absolute inset-0 pointer-events-none" style={{
        backgroundImage: `linear-gradient(${WHITE}08 1px, transparent 1px), linear-gradient(90deg, ${WHITE}08 1px, transparent 1px)`,
        backgroundSize: "60px 60px",
      }} />

      <div className="max-w-[1300px] mx-auto px-6 md:px-12 lg:px-20">
        <div className="text-center mb-16">
          <span className="block text-[10px] tracking-[0.45em] uppercase mb-4 font-seitu" style={{ color: `${PRIMARY}` }}>By The Numbers</span>
          <h2 className="font-faculty text-4xl md:text-5xl text-white" style={{ fontWeight: 400 }}>
            A Decade of <span style={{ color: PRIMARY }}>Global Impact</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, i) => (
            <motion.div
              key={i}
              className="numbers-item text-center opacity-0"
              whileHover={{ scale: 1.05 }}
            >
              <div
                className="mx-auto mb-5 flex items-center justify-center rounded-2xl"
                style={{ width: 80, height: 80, background: `${WHITE}08`, border: `1px solid ${WHITE}15` }}
              >
                <span className="font-faculty text-2xl" style={{ color: PRIMARY }}>{stat.value}</span>
              </div>
              <div className="text-[11px] tracking-[0.3em] uppercase font-seitu" style={{ color: `${WHITE}80` }}>{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-[900px] mx-auto px-6 md:px-12 text-center">
        <div className="vision-content opacity-0">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <motion.a
              href="#"
              className="inline-flex items-center gap-3 px-8 py-4 text-[11px] tracking-[0.3em] uppercase font-seitu text-white rounded-xl"
              style={{ background: MSU_GREEN }}
            ...
            <motion.a
              href="#"
              className="inline-flex items-center gap-3 px-8 py-4 text-[11px] tracking-[0.3em] uppercase font-seitu rounded-xl"
  ...
  return (
    <main style={{ background: CHINESE_BLACK }}>
      ...
      <RegionSection
        ...
        cover={UK_COVER}
        projects={UK_PROJECTS}
        description="From the elegant streets of Chelsea to the regeneration corridors of Manchester, Fakhruddin's UK portfolio reflects an acute understanding of British real estate — blending heritage sensibility with contemporary design."
        accent={PRIMARY}
        flipped={false}
      />
      <RegionSection
        region="Uganda"
        cover={UGANDA_COVER}
        projects={UGANDA_PROJECTS}
        description="East Africa's fastest-growing cities are being transformed by bold, purposeful investment. Fakhruddin's Uganda portfolio captures this momentum — delivering residential and commercial developments that meet rising demand with lasting quality."
        accent={DEEP_AQUAMARINE}
        flipped={true}
      />
